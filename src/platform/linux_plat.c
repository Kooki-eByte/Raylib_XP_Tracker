...
char *get_save_dir(const char *appname) {
  ...
  if (xdg != NULL && xdg[0] != '\0') {
    ...
    size_t len = strlen(home) + strlen(XP_TRACKER_SAVE_DATA_LOC) + 20;
    path = malloc(len);
    snprintf(path, len, "%s/.local/share/%s", home, XP_TRACKER_SAVE_DATA_LOC);
  }
...
bool create_directory() {
  ...
  if (stat(dir, &st) == -1) {
    if (mkdir(dir, 0700) == -1) {
      int err_msg = errno;
      fprintf(stderr,
              "Failed to create xp tracker directory at location: %s - err "
              "code: %d\n",
      ...
      if (errno) {
        fprintf(stderr,
                ...
                "search permission. Err Message code: %d\n",
                err_msg);
      }
